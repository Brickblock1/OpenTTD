...
private:
	...
	void BuildTunnel(TunnelType type)
	{
		...
		Command<Commands::BuildTunnel>::Post(STR_ERROR_CAN_T_BUILD_TUNNEL_HERE, CcBuildTunnel,
	...
	void SortTunnelsList()
	{
		...
		this->GetWidget<NWidgetCore>(WID_BTS_DROPDOWN_CRITERIA)->SetString(BuildTunnelWindow::sorter_names[this->tunnels.SortType()]);
	 ...
	 * @return the StringID.
	 */
	std::string GetTunnelSelectString(const BuildTunnelData &tunnel_data) const
	{
		...
		if (tunnel_data.spec->speed == UINT16_MAX) {
			return _game_mode == GM_EDITOR
				? GetString(STR_SELECT_TUNNEL_INFO_NAME, tunnel_data.spec->description)
				: GetString(STR_SELECT_TUNNEL_INFO_NAME_COST, tunnel_data.spec->description, tunnel_data.cost);
		}

		uint64_t packed_velocity = PackVelocity(tunnel_data.spec->speed, static_cast<VehicleType>(this->transport_type));
		return _game_mode == GM_EDITOR
			? GetString(STR_SELECT_TUNNEL_INFO_NAME_MAX_SPEED, tunnel_data.spec->description, packed_velocity)
			: GetString(STR_SELECT_TUNNEL_INFO_NAME_MAX_SPEED_COST, tunnel_data.spec->description, packed_velocity, tunnel_data.cost);
	}
...
public:
		...
		tunnels(std::move(tl))
	{
		...
		this->GetWidget<NWidgetCore>(WID_BTS_CAPTION)->SetString((transport_type == TRANSPORT_ROAD) ? STR_SELECT_ROAD_TUNNEL_CAPTION : STR_SELECT_RAIL_TUNNEL_CAPTION);
		...
		switch (widget) {
			...
			case WID_BTS_TUNNEL_LIST: {
				...
				for (const BuildTunnelData &tunnel_data : this->tunnels) {
					...
					/* Tunnels have different sprites depending on transport type. */
					switch (transport_type)
					{
					case TRANSPORT_RAIL:

						/* The first tunnel type is always the rail type provided one. */
...
static const NWidgetPart _nested_build_tunnel_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL),
		...
		NWidget(WWT_CAPTION, COLOUR_DARK_GREEN, WID_BTS_CAPTION), SetStringTip(STR_SELECT_RAIL_TUNNEL_CAPTION, STR_TOOLTIP_WINDOW_TITLE_DRAG_THIS),
	...
	NWidget(NWID_HORIZONTAL),
		NWidget(NWID_VERTICAL),
			...
			NWidget(NWID_HORIZONTAL),
				NWidget(WWT_TEXTBTN, COLOUR_DARK_GREEN, WID_BTS_DROPDOWN_ORDER), SetFill(1, 0), SetStringTip(STR_BUTTON_SORT_BY, STR_TOOLTIP_SORT_ORDER),
				NWidget(WWT_DROPDOWN, COLOUR_DARK_GREEN, WID_BTS_DROPDOWN_CRITERIA), SetFill(1, 0), SetToolTip(STR_TOOLTIP_SORT_CRITERIA),
			EndContainer(),
			/* Matrix. */
...
void ShowBuildTunnelWindow(TileIndex tile, TileIndex tile2, TransportType transport_type, uint8_t road_rail_type)
{
	...
	/* The tunnel length without portals*/
	...
	if (_ctrl_pressed && CheckTunnelAvailability(last_tunnel_type, tunnel_len + 2).Succeeded()) {
		Command<Commands::BuildTunnel>::Post(STR_ERROR_CAN_T_BUILD_TUNNEL_HERE, CcBuildTunnel, tile, transport_type, last_tunnel_type, road_rail_type);
		return;
	}
	
	/* only query tunnel building possibility once, result is the same for all tunnels!
	 * returns CMD_ERROR on failure, and price on success */
	CommandCost ret = Command<Commands::BuildTunnel>::Do(CommandFlagsToDCFlags(GetCommandFlags<Commands::BuildTunnel>()) | DoCommandFlag::QueryCost, tile, transport_type, 0, road_rail_type);
	...
	if (!ret.Failed()) {
		...
		for (TunnelType tun_type = 0; tun_type != MAX_TUNNELS; tun_type++) {
			...
			if (type_check.Succeeded()) {
				 ...
				 * tunnel itself (not computed with DoCommandFlag::QueryCost) */
				item.cost = ret.GetCost() + (((int64_t)tot_tunneldata_len * _price[Price::BuildTunnel] * item.spec->price) >> 8) + infra_cost;
				any_available = true;
			}
		}
	...
	if (tl.size() == 1) { 
		Command<Commands::BuildTunnel>::Post(STR_ERROR_CAN_T_BUILD_TUNNEL_HERE, CcBuildTunnel, tile, transport_type, tl.emplace_back().index, road_rail_type);
		return;
	} else if (!tl.empty()) {
		new BuildTunnelWindow(_build_tunnel_desc, tile, transport_type, road_rail_type, std::move(tl));
	} else {
		ShowErrorMessage(GetEncodedString(STR_ERROR_CAN_T_BUILD_TUNNEL_HERE), TileX(tile2) * TILE_SIZE, TileY(tile2) * TILE_SIZE, ret);
	}
}
